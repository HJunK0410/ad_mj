...
COCO_PATH="/home/user/hyunjun/AD/data"  # ⚠️ 여기를 실제 데이터셋 경로로 수정하세요!
OUTPUT_DIR="./outputs/new_sjpm/"
BATCH_SIZE=4
EPOCHS=100
NUM_WORKERS=4
...
# 0.01 간격으로 그리드 서치 수행
LAMBDA_CONT_2D_VALUES=(0.02 0.04 0.06 0.08 0.10)  # 2D continuity loss 가중치 범위
LAMBDA_CONT_1D_VALUES=(0.02 0.04 0.06 0.08 0.10)  # 1D continuity loss 가중치 범위
NORM_MASK_BBOX_COEF_VALUES=(0.1 0.2 0.3)           # Bbox 영역의 norm mask 계수 (튜닝 가능)
...
# 그리드 서치 시작
TOTAL_COMBINATIONS=$((${#LAMBDA_CONT_2D_VALUES[@]} * ${#LAMBDA_CONT_1D_VALUES[@]} * ${#NORM_MASK_BBOX_COEF_VALUES[@]}))
CURRENT_COMBINATION=0

echo "============================================================"
echo "DETR 그리드 서치 시작 (Continuity Loss + Norm Mask Bbox Coef)"
echo "============================================================"
...
for LAMBDA_CONT_2D in "${LAMBDA_CONT_2D_VALUES[@]}"; do
    for LAMBDA_CONT_1D in "${LAMBDA_CONT_1D_VALUES[@]}"; do
        for NORM_MASK_BBOX_COEF in "${NORM_MASK_BBOX_COEF_VALUES[@]}"; do
            ...
            CURRENT_OUTPUT_DIR="${OUTPUT_DIR}_2d${LAMBDA_2D_STR}_1d${LAMBDA_1D_STR}_nm${NORM_MASK_BBOX_COEF_STR}"
