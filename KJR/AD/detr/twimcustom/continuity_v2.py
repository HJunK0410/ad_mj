...
class ContinuityHead2D(nn.Module):
    ...
    def forward(self, x, norm_mask=None):
        ...
        z_hat: [B, D, H, W] — causal conv2d 예측 결과 (2개 방향 평균)
        """
        z = self.proj(x)          # [B, D, H, W]
        
        # 2개 방향에 대해 계산: 원본, 좌우 flip
        z_hat_list = []
        
        # 1. 원본 방향
        z_hat_orig = self.masked_conv(z)
        z_hat_orig = self.relu(z_hat_orig)
        z_hat_orig = self.conv1x1(z_hat_orig)
        z_hat_list.append(z_hat_orig)
        
        # 2. 좌우 flip (flip dim=-1)
        z_flip_lr = torch.flip(z, dims=[-1])
        z_hat_flip_lr = self.masked_conv(z_flip_lr)
        z_hat_flip_lr = self.relu(z_hat_flip_lr)
        z_hat_flip_lr = self.conv1x1(z_hat_flip_lr)
        z_hat_flip_lr = torch.flip(z_hat_flip_lr, dims=[-1])  # flip back
        z_hat_list.append(z_hat_flip_lr)
        
        # 2개 방향의 합
        z_hat = torch.stack(z_hat_list, dim=0).sum(dim=0)  # [B, D, H, W]
