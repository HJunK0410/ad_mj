    ...
    def __init__(self, img_folder, label_folder, transforms, return_masks=False):
        ...
        for im_file in self.im_files:
            if self.label_folder:
                ...
                # 또는 image/ -> label/ 변환
                im_path_str = str(im_file)
                label_file = Path(im_path_str.replace('/images/', '/labels/').replace('\\images\\', '\\labels\\'))
                if not label_file.exists():
                    # image/ -> label/ 변환 시도
                    label_file = Path(im_path_str.replace('/image/', '/label/').replace('\\image\\', '\\label\\'))
                if not label_file.exists():
                    # Fallback: 같은 폴더에서 찾기
...
def make_yolo_transforms(image_set):
    ...
    # COCO transforms 재사용
    # 'test'는 'val'과 동일한 transform 사용
    from .coco import make_coco_transforms
    transform_image_set = 'val' if image_set == 'test' else image_set
    return make_coco_transforms(transform_image_set)


def build_yolo(image_set, args):
    """Build YOLO format dataset (v1과 동일한 방식, yaml 없이도 작동)."""
    # 이미지 경로와 라벨 경로가 직접 지정된 경우
    if hasattr(args, 'image_path') and args.image_path is not None:
        img_folder = Path(args.image_path)
        label_folder = Path(args.label_path) if (hasattr(args, 'label_path') and args.label_path is not None) else None
        
        if not img_folder.exists():
            raise FileNotFoundError(f"이미지 경로가 존재하지 않습니다: {img_folder}")
        ...
        dataset = YOLODataset(
            ...
            transforms=make_yolo_transforms(transform_image_set),
            return_masks=args.masks
        )
        return dataset
    
    # 기존 방식: coco_path에서 자동으로 찾기
