...
def get_args_parser():
    ...
    parser.add_argument('--alpha', default=0.3, type=float,
                        help="Weight for 1st-order differencing loss (default: 0.3)")
    parser.add_argument('--beta', default=1.0, type=float,
                        help="Weight for 2nd-order differencing loss (default: 1.0)")
...
def main(args):
    ...
    # eval 모드일 때는 test 데이터셋 사용, 아니면 train/val 사용
    if args.eval:
        # eval 모드: test 데이터셋 사용
        dataset_train = None
        dataset_val = build_dataset(image_set='test', args=args)
    else:
        # 학습 모드: train/val 데이터셋 사용
