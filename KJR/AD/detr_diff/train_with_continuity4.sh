...
TRAIN_IMAGE_DIR="/home/user/hyunjun/AD/data/images/train"
TRAIN_LABEL_DIR="/home/user/hyunjun/AD/data/labels/train"
VAL_IMAGE_DIR="/home/user/hyunjun/AD/data/images/val"
VAL_LABEL_DIR="/home/user/hyunjun/AD/data/labels/val"
OUTPUT_DIR="./outputs/detr_diff_100/"
BATCH_SIZE=4
EPOCHS=100
NUM_WORKERS=4
...
# 0.01 간격으로 그리드 서치 수행
LAMBDA_CONT_2D_VALUES=(0.02)  # 2D continuity loss 가중치 범위
LAMBDA_CONT_1D_VALUES=(0.06)  # 1D continuity loss 가중치 범위
NORM_MASK_BBOX_COEF_VALUES=(0.2)           # Bbox 영역의 norm mask 계수 (튜닝 가능)
ALPHA_VALUES=(0.1 0.3 0.5)                          # 1차 차분 가중치 (기본값: 0.3)
BETA_VALUES=(0.5 1.0 1.5 2.0)                           # 2차 차분 가중치 (기본값: 1.0)
...
# 그리드 서치 시작
TOTAL_COMBINATIONS=$((${#LAMBDA_CONT_2D_VALUES[@]} * ${#LAMBDA_CONT_1D_VALUES[@]} * ${#NORM_MASK_BBOX_COEF_VALUES[@]} * ${#ALPHA_VALUES[@]} * ${#BETA_VALUES[@]}))
CURRENT_COMBINATION=0

echo "============================================================"
echo "DETR 그리드 서치 시작 (Continuity Loss + Norm Mask Bbox Coef + Alpha/Beta)"
echo "============================================================"
...
echo "Norm Mask Bbox Coef 범위: ${NORM_MASK_BBOX_COEF_VALUES[@]}"
echo "Alpha 범위: ${ALPHA_VALUES[@]}"
echo "Beta 범위: ${BETA_VALUES[@]}"
echo "============================================================"
echo ""
...
for LAMBDA_CONT_2D in "${LAMBDA_CONT_2D_VALUES[@]}"; do
    for LAMBDA_CONT_1D in "${LAMBDA_CONT_1D_VALUES[@]}"; do
        for NORM_MASK_BBOX_COEF in "${NORM_MASK_BBOX_COEF_VALUES[@]}"; do
