#!/usr/bin/env python3
"""
crack-seg 데이터셋에 대한 YOLOv8 Detection 학습 스크립트
- baseline: 순수 YOLO (NPP loss 없음)
- best: 기존 실험에서 가장 좋았던 NPP loss 하이퍼파라미터 적용
  (l2d=0.04, l1d=0.02, mask=0.2, fpn=15,18,21 / mAP50-95=0.46062)
"""
...
# train_npp_l2d0.04_l1d0.02_mask0.2_fpn15_18_21 -> mAP50-95=0.46062
# ============================================================
BEST_HP = {
    'npp_lambda_2d': 0.04,
    'npp_lambda_1d': 0.02,
    'npp_bbox_mask_weight': 0.2,
    'npp_fpn_sources': '15,18,21',
}
...
def parse_args():
    """명령줄 인자 파싱"""
    experiment_type = 'baseline'  # baseline 또는 best
    device = '0'
    batch = 16
    epochs = 100
    imgsz = 640
...
if experiment_type == 'best':
    npp_params = dict(BEST_HP)
    PROJECT_NAME = 'crack_best_npp'
    print(f"[crack-seg] Best NPP HP 실험 시작: {npp_params}")
else:
    npp_params = dict(BASELINE_HP)
    PROJECT_NAME = 'crack_baseline'
    print(f"[crack-seg] Baseline 실험 시작 (NPP loss 비활성화)")
...
class CustomDetectionTrainer(DetectionTrainer):
    ...
    def get_validator(self):
        ...
        for name in ['npp_lambda_2d', 'npp_lambda_1d', 'npp_bbox_mask_weight', 'npp_fpn_sources']:
